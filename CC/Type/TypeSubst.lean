...
def PType.tsubst (S : PType n m1) (σ : TypeMap n m1 m2) : PType n m2 :=
  ...
  | PType.arr D (CType.capt C1 S1) (CType.capt C2 S2) =>
    PType.arr D (CType.capt C1 (S1.tsubst σ)) (CType.capt C2 (S2.tsubst σ.ext_var))
  ...
  | PType.ref S =>
    PType.ref (S.tsubst σ)
  | PType.reader S =>
    PType.reader (S.tsubst σ)
...
theorem PType.rename_tsubst (T : PType n1 m1)
  ...
  | PType.boxed (CType.capt C1 S1) => by
    ...
    simp [rename, tsubst, ih1]
  | PType.ref S => by
    have ih := PType.rename_tsubst S (σ := σ) (f := f) (g := g)
    simp [rename, tsubst, ih]
  | PType.reader S => by
    have ih := PType.rename_tsubst S (σ := σ) (f := f) (g := g)
    simp [rename, tsubst, ih]
...
theorem PType.tsubst_rename (T : PType n1 m1)
  ...
  | PType.boxed (CType.capt C1 S1) => by
    ...
    simp [rename, tsubst, ih1]
  | PType.ref S => by
    have ih := PType.tsubst_rename S (f := f) (g := g) (σ := σ)
    simp [rename, tsubst, ih]
  | PType.reader S => by
    have ih := PType.tsubst_rename S (f := f) (g := g) (σ := σ)
    simp [rename, tsubst, ih]
...
@[simp]
lemma tsubst_arr :
  (PType.arr D T U).tsubst f = PType.arr D (T.tsubst f) (U.tsubst f.ext_var) := by
  cases T; cases U
  simp [PType.tsubst]
  simp [CType.tsubst]
...
lemma PType.tsubst_comp (T : PType n m1) (g1 : TypeMap n m1 m2) (g2 : TypeMap n m2 m3) :
  ...
  | PType.boxed (CType.capt C1 S1) => by
    ...
    simp [tsubst, ih]
  | PType.ref S => by
    have ih := PType.tsubst_comp S g1 g2
    simp [tsubst, ih]
  | PType.reader S => by
    have ih := PType.tsubst_comp S g1 g2
    simp [tsubst, ih]
