...
| cons_empty {γ : Store 0} :
  TypedStore γ Γ ->
  Typed Γ t C T ->
  TypedStore (Store.cons γ ⟨t, isVal⟩) (Ctx.extend_var Γ T Region.glob)
| cons {γ : Store (Nat.succ n)} :
  TypedStore γ Γ ->
  Typed Γ t C T ->
  TypedStore (Store.cons γ ⟨t, isVal⟩) (Ctx.extend_var Γ T (Region.var 0))
